 ...
 *
 * NOT RUN IN CI, and excluded from jest by libraries/database/jest.config.cjs
 * (testPathIgnorePatterns matches *.integration.test.ts). What these assert —
 * a real INSERT race collapsing to one row, a partial unique index rejecting a
 * second owner, a live Auto-Discovery query — is Postgres behaviour. Mocking
 * the Supabase client would leave the tests asserting the mock instead, so they
 * stay on node:test and are run by hand against a local stack.
 *
 ...
 *
 * Run:
 *   pnpm exec tsx --test libraries/database/src/claim.integration.test.ts
 *
 ...
 *
 * The pure helpers that used to share this file (normalizeHandle,
 * decideInitialClaimKind) now live in claim.test.ts, which jest DOES run.
 */
...
async function ensureCreator(displayName: string): Promise<string> {
  ...
  if (ins.error || !ins.data)
    throw new Error(`creator insert: ${ins.error?.message}`);
...
describe('findOrCreateProfile — concurrent inserts', () => {
  it('produces exactly one profile row when many requests fire same URL at once', async () => {
    ...
    assert.equal(
      profileIds.size,
      1,
      'all concurrent callers must converge on one profile',
    );
    assert.equal(
      createdCount,
      1,
      'exactly one INSERT should succeed; others must be no-op SELECTs',
    );
  });
...
describe('addProfileClaim + DB constraints', () => {
  ...
  it('partial unique index blocks a second owner on the same profile', async () => {
    ...
    assert.notEqual(
      dup.error,
      null,
      'second owner on same profile must violate partial unique index',
    );
  });
});
...
describe('findCandidatesByHandle — auto-discovery', () => {
  it('finds cross-platform matches and excludes seed platform', async () => {
    ...
    assert.ok(
      ...
      'seed platform must be excluded',
    );
  ...
  it('hides profiles already owned by another user (privacy guard)', async () => {
    ...
    assert.equal(
      ttFound,
      undefined,
      'profile owned by another user must be hidden from discovery',
    );
  });
});
