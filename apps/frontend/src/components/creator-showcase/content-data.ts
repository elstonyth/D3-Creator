...
export function formatCompact(n: number, locale: Locale = 'en'): string {
...
export function formatExact(n: number, locale: Locale = 'en'): string {
...
export function formatPostDate(iso: string, locale: Locale = 'en'): string {
...
export function formatPostDateLong(iso: string, locale: Locale = 'en'): string {
  const t = Date.parse(iso);
  return Number.isNaN(t)
    ? ''
    : new Intl.DateTimeFormat(locale === 'zh' ? 'zh-CN' : 'en-GB', {
...
export function formatPrimaryMetric(
  ...
  if (post.metrics.views != null) {
    return {
      label: locale === 'zh' ? '播放量' : 'views',
      value: formatCompact(post.metrics.views, locale),
    };
  }
  return {
    label: locale === 'zh' ? '点赞' : 'likes',
    value: formatCompact(post.metrics.likes, locale),
  };
}
