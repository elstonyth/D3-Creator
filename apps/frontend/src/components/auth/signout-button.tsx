  ...
  return (
    <div className="inline-flex items-center gap-2">
      <button
        ...
        onClick={handle}
        disabled={pending}
        className="px-3 py-1.5 rounded-md text-label text-fg-muted hover:text-fg hover:bg-white/[0.04] transition-colors disabled:opacity-50"
      >
        {pending ? 'Signing out…' : 'Sign out'}
      </button>
      {error ? (
        <span role="alert" className="text-caption text-fg-muted">
          ⚠ {error}
        </span>
      ) : null}
    </div>
