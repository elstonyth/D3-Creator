...
const RAW_TOP_CREATORS: Array<
  Omit<CreatorRow, 'totalViews' | 'totalEngagement'>
> = [
  {
    rank: 1,
    handle: '@miawatkins',
  ...
  {
    rank: 11,
    handle: '@ramen.bookmark',
    primaryPlatform: 'tiktok',
    followers: 88_200,
    growth30d: 12_870,
    engagementRate: 0.098,
  },
  {
    rank: 12,
    handle: '@ailin.studio',
 ...
 */
export const DEMO_VIEWS: Record<
  PlatformFilter,
  { views30d: number; viewsLifetime: number }
> = {
...
export function getCreatorsForFilter(filter: PlatformFilter): CreatorRow[] {
  ...
  return TOP_CREATORS.filter((c) => c.primaryPlatform === filter).map(
    (c, i) => ({ ...c, rank: i + 1 })
  );
}
...
export function summarize(filter: PlatformFilter): LeaderboardSummary {
  ...
  const combinedEngagement = base.reduce(
    (acc, c) => acc + c.totalEngagement,
    0
  );
 ...
 */
export function formatShowcase(n: number, locale: Locale = 'en'): string {
  return locale === 'zh'
...
export function placeholderDeltaPct(
  value: number,
  lo = 0.03,
  hi = 0.09
): number {
