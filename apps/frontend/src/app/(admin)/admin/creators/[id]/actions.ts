...
export async function renameCreator(
  ...
  try {
    ...
    const { error } = await admin
      .from('creator')
      .update({ display_name: nameRes.value })
      .eq('id', creatorId);
    if (error) {
...
export async function addCreatorUrl(
  ...
  try {
    ...
    if (
      !profileRes.value.created &&
      profileRes.value.profile.creator_id !== creatorId
    ) {
      return {
        ok: false,
...
export async function addCreatorLogin(
  ...
  try {
    ...
    if (existingLogin.error) {
      console.error(
        '[admin/addCreatorLogin] creator_link lookup',
        existingLogin.error,
      );
      return {
        ok: false,
    ...
    if (linked.error) {
      console.error(
        '[admin/addCreatorLogin] creator_link upsert',
        linked.error,
      );
      return {
        ok: false,
...
export async function deleteCreator(
  ...
  try {
    ...
    for (const l of (links.data ?? []) as { user_id: string }[]) {
      ...
      if (delErr)
        console.error('[admin/deleteCreator] deleteUser', l.user_id, delErr);
    }
